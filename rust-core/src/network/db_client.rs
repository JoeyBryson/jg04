...
impl NwDbClient {
    ...
    pub async fn set_profile(&self, profile: NwProfile) -> Result<()> {
        let (tx, rx) = oneshot::channel();

        self.send_request(
            NwDbRequest::SetProfile {
                profile,
                reply: tx,
            },
            rx,
    ...
    pub async fn get_profile(&self) -> Result<NwProfile> {
        let (tx, rx) = oneshot::channel();

        let profile = self.send_request(
            NwDbRequest::GetProfile {
                reply: tx,
            },
            rx,
        )
