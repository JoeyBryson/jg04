...
db_requests! {
    ...
    writes {
        ...
        AddNwMessage(message: NwMessage) -> () => add_nw_message { async add_nw_message }
            emits [crate::notifications::UiEvent::ChatDataChanged { topic_id: hex::encode(message.topic_id) }, crate::notifications::UiEvent::ChatHeadersChanged];
        AddNwContact(contact: NwContact) -> () => add_nw_contact { async add_nw_contact, sync add_nw_contact_sync }
            emits [crate::notifications::UiEvent::ContactsChanged];
        AddNwChat(chat: NwChat) -> () => add_nw_chat { async add_nw_chat, sync add_nw_chat_sync }
            emits [crate::notifications::UiEvent::ChatHeadersChanged];
    }
}
