...
#[uniffi::export]
pub fn add_sample_messages(db_path_string: String) -> Result<(), FfiError> {
    let manager = DbManager::spawn(db_path_string)?;
    let client = manager.spawn_client();

    let rt = Runtime::new().map_err(|e| FfiError::Internal { msg: e.to_string() })?;

    rt.block_on(async move { client.add_sample_chat().await })
        .map_err(|e| FfiError::Internal {
...
#[uniffi::export]
pub fn add_sample_message(client: Arc<DbClient>, time: i32) -> Result<(), FfiError> {
    let rt = Runtime::new().map_err(|e| FfiError::Internal { msg: e.to_string() })?;

    rt.block_on(async move { client.add_sample_message(time).await })
        .map_err(|e| FfiError::Internal {
...
#[uniffi::export]
pub fn add_sample_data(db_path_string: String) -> Result<(), FfiError> {
    let manager = DbManager::spawn(db_path_string)?;
    ...
    rt.block_on(async move { client.add_sample_data().await })
        .map_err(|e| FfiError::Internal {
            msg: format!("Failed to add sample data: {}", e),
        })?;
...
fn add_contact_id(
    ...
    let endpoint_id = EndpointId::from_bytes(&bytes).map_err(anyhow::Error::from)?;

    let contact = NwContact { name, endpoint_id };
